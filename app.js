...
const districtSnakeToCamel=(newObject)=>{
 ...
 app.get("/states/",async (request,response)=>{
     ...
     const statesResult=statesList.map((eachObject)=>{
         return
         objectSnakeToCamel(eachObject);

     });
     response.send(statesResult);
 });

 //2.Return a State based on the state ID
...
//4.return a district based on the district ID
...
//5. delete a district from the district table based on the district ID
...
//6.update the details of the specific district based on the district ID
...
// 8.return a state name based on the district ID
app.get("/districts/:districtId/details/",async (request,response)=>{
    const {districtId}=request.params;
    const stateDetails=`SELECT state_name FROM state JOIN district ON
